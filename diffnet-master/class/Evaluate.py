...
class Evaluate():
    ...
    def evaluateRankingPerformance(self, evaluate_index_dict, evaluate_real_rating_matrix, \
        ...
        batch_size = len(user_list) / num_procs  # 每次传播的batch
        ...
        return np.mean(hr_list), np.mean(ndcg_list)  # HR, NDCG

    def getHrNdcgProc(self, 
        evaluate_index_dict, 
        evaluate_real_rating_matrix,  # positive
        evaluate_predict_rating_matrix,  # negative
        topK, 
        user_list):
        ...
        for u in user_list:
            real_item_index_list = evaluate_index_dict[u]  # u用户关联哪些产品
            real_item_rating_list = list(np.concatenate(evaluate_real_rating_matrix[real_item_index_list]))  # 真实值，长度为u关联的产品个数
            positive_length = len(real_item_rating_list)
            target_length = min(positive_length, topK)
           
            predict_rating_list = evaluate_predict_rating_matrix[u]  # 1000个预测值
            real_item_rating_list.extend(predict_rating_list)  # 真实值和预测值集合在一起，然后排序（把序号排出来，也就是arg）
