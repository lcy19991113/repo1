...
class diffnet():
    ...
    def startConstructGraph(self):
        self.initializeNodes()  # 计算user_embedding，item_embedding, user_review, item_review_vetor
        self.constructTrainGraph()
        self.saveVariables()
        self.defineMap()

    def inputSupply(self, data_dict):  # 构造关系矩阵和consumeItem矩阵
        ...
        self.social_neighbors_dense_shape = np.array([self.conf.num_users, self.conf.num_users]).astype(np.int64)  # 这俩是随机初始化的矩阵，好像没用，只是为了初始化sparse矩阵
    ...
    def generateUserEmbeddingFromSocialNeighbors(self, current_user_embedding):
        user_embedding_from_social_neighbors = tf.sparse_tensor_dense_matmul(
            self.social_neighbors_sparse_matrix, current_user_embedding  # （17237*17237）*（17237*32）
        )
    ...
    def initializeNodes(self):
        ...
        self.user_embedding = tf.Variable(
            tf.random_normal([self.conf.num_users, self.conf.dimension], stddev=0.01), name='user_embedding')  # 17237*32
        self.item_embedding = tf.Variable(
            tf.random_normal([self.conf.num_items, self.conf.dimension], stddev=0.01), name='item_embedding')  # 38342*32

        self.user_review_vector_matrix = tf.constant(\
            np.load(self.conf.user_review_vector_matrix), dtype=tf.float32)  # 17237*150
        self.item_review_vector_matrix = tf.constant(\
            np.load(self.conf.item_review_vector_matrix), dtype=tf.float32)  # 38324*150
        ...
        self.my_reduce = tf.layers.Dense(\
            self.conf.dimension, activation=tf.nn.sigmoid, name='reduce_dimension')
    ...
    def constructTrainGraph(self):
        ...
        user_embedding_from_consumed_items = self.generateUserEmebddingFromConsumedItems(self.final_item_embedding)  # consumed_items_sparse_matrix * current_item_embedding （17237*38324）*（38324*32）
        ...
        # self.final_user_embedding = self.my_reduce(tf.concat([second_gcn_user_embedding, user_embedding_from_consumed_items], axis=1))  # 32*2 -> 32
        ...
        self.prediction = tf.sigmoid(tf.reduce_sum(predict_vector, 1, keepdims=True))  # inner product，sigmoid
    ...
    def saveVariables(self):
        ...
        for v in self.reduce_dimension_layer.variables:  # 这是全连接层，从50->32的
